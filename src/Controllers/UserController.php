...
class UserController extends Controller
{
    ...
    protected function saveUser(Request $request)
    {
        ...
        $validation = \Validator::make($request->all(), with(new UserValidation)->getRules());

        if ($validation->fails()) {
            return redirect()->route('extensionsvalley.admin.adduser',['accesstoken'=>$request->input('accesstoken')])->withErrors($validation)->withInput();
        }
    ...
    public function updateUser(Request $request)
    {
        ...
        if ($validation->fails()) {
            return redirect()->route('extensionsvalley.admin.edituser', ['id' => $user->id,'accesstoken'=>$request->input('accesstoken')])->withErrors($validation)->withInput();
        }
