...
class ExtensionController extends Controller
{
    ...
    public function getIndex(Request $request)
    {
        ...
        if($request->has('status')){
            $extensions = $extensions->Where('status',$request->input('status'));
        }
    ...
    public function disablePackage($package_id){
        ...
        if(trim($error_message) != ""){
            ...
            $this->RemoveACLData(['vendor' => strtolower($packageinfo->vendor),'name' => strtolower($packageinfo->name)]);
            return redirect()->route('extensionsvalley.admin.manageextension')->with(['warning' => 'Package Deactivated successfully!' ] );
        }
    }

    public function RemoveACLData($data){
        ...
        if(isset($menugroups[$module_key])){
             $acl_keys[] = $menugroups[$module_key]['acl_key'];
             foreach($menugroups[$module_key]['sub_menu'] as $subkey){
                $acl_keys[] = $subkey['acl_key'];
             }
         }

         \DB::table('acl_permission')->WhereIn('acl_key',$acl_keys)->delete();
    ...
    public function uninstallPackage($package_id){
        ...
        if($packageinfo->package_type == "laflux-theme"){
            ...
            ##Remove resoucre views of theme
