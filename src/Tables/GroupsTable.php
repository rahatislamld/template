...
class GroupsTable extends BaseTable
{
    ...
    public function getQuery($request)
    {
        ...
        return datatables()->of($groups)
            ...
            ->addColumn('status', '@if($status==1) <span class="glyphicon glyphicon-ok"> Published</span> @else <span class="glyphicon glyphicon-remove"> Unpublished</span> @endif')
            ->addColumn('created_at', '{{date("M-j-Y",strtotime($created_at))}}')
            ->rawColumns(['sl', 'status','created_at'])
            ->make(true);
    }
